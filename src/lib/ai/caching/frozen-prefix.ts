...
export function buildFrozenSystemPrompt(campaign?: CampaignContext): string {
  ...
  // Используем аккуратные границы слов через lookbehind/lookahead, чтобы не портить
  // обычные слова русского языка (похитители, сохраненный, хитрость).
  return rawPrompt
    .replace(/\bHP\b/g, "здоровье")
    .replace(/(?<![а-яёА-ЯЁ])(?:хиты?|хитов|хитам|хитах|хитами)(?![а-яёА-ЯЁ])/gi, "очков здоровья")
    .replace(/(?<![а-яёА-ЯЁ])(?:ранен[а-яё]*|ранения?|ранений)(?![а-яёА-ЯЁ])/gi, "травмирован");
}
